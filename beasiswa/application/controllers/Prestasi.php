...
class Prestasi extends CI_Controller{
	...
	function lihat()
	{
		...
		$data['history'] = $this->M_General->showHistory($id_history);
    ...
    function lihat1()
	{
		...
		$data['id_history'] = $id_history;
		$data['history'] = $this->M_General->showHistory($id_history);
        $this->template->load('template','prestasi/viewlist1',$data);		
     }

     function profil(){
     	$username = $this->session->userdata('username');
		$data['mahasiswa']  = $this->M_General->showMahasiswa($username);
     	$this->template->load('template','mahasiswa/profil', $data);
     }
     ...
     function updateVerifikasi(){
     		...
     		$status == '1' ? $status='Sudah Terverifikasi': $status='Belum Terverifikasi';
     		$data = array(
     			'status' => $status
     			); 		
     		if($this->M_Prestasi->updateVerifikasi($id_history,$data))
     			$this->session->set_flashdata('pesan','Berhasil Diubah');
     		else
     			$this->session->set_flashdata('pesan','Gagal Diubah');
     		redirect('prestasi/lihat/'.$id_history);
    ...
    echo '
		<div style="width:40%; margin-bottom:3%;">
			...
			<div style="width:70%; ">
				'.$user.'<br>'.$mhs["nama_mahasiswa"].'<br>'.$mhs["jurusan"].'
			</div>
	...
	function formPrestasi($id_prestasi){
		...
		$data .= '
            ...
            <div class="form-group">
                ...
                <div class="col-sm-9">
                    <input type="text" name="prestasi" required placeholder="Masukan Prestasi" id="form-field-1" class="form-control">
                </div>
            </div>
		...
		else
		{
			...
			$data .= '
			    ...
			    <div class="form-group">
	                ...
	                <div class="col-sm-9">
	                    <input type="text" name="organisasi" required placeholder="Masukan Nama Organisasi" id="form-field-1" class="form-control">
	                </div>
	            </div>
			    <div class="form-group">
	                <label class="col-sm-2 control-label" for="form-field-1">
    ...
    function updateStatus(){
    	if(isset($_POST['submit'])){
			$this->M_Dosen->saveStatus();
			redirect('dosen');
		}else{
			$this->template->load('template','dosen/viewlist');
		}
    }
